"""
Helper functions for check_signal.py.

Contains model loading, data pair generation, activation collection,
signal metric computation, and signal evaluation.
"""
...
from wisent.core.utils.config_tools.constants import NORM_EPS, DEFAULT_RANDOM_SEED, DIAGNOSTIC_MLP_HIDDEN_SIZES, KMEANS_N_INIT_LARGE
...
def evaluate_signal(results):
    ...
    threshold_checks = [
        ("Linear cone (cosine > 0.2)", "diff_cosine_mean", 0.2, True),
        ("Dominant direction (PC1 > 30%)", "pc1_variance", 0.3, True),
        ("Cluster separation (K-means > 70%)", "kmeans_accuracy", 0.7, True),
        ("Nonlinear separation (MLP > 70%)", "mlp_accuracy", 0.7, True),
        ("Kernel separation (SVM > 70%)", "svm_accuracy", 0.7, True),
        ("Stable signal (std < 0.05)", "subset_stability", 0.05, False),
    ]
    checks = []
    for name, key, threshold, above in threshold_checks:
        value = results[key]
        passed = value > threshold if above else value < threshold
        checks.append((name, passed, value))
    return checks
