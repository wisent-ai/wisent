...
def load_boolq_questions(limit, preferred_doc: str = "validation"):
    ...
    if not docs:
        raise RuntimeError(f"Could not load BoolQ documents from {preferred_doc} source")
...
def evaluate_baseline_performance(
    ...
    questions = load_boolq_questions(limit=num_questions, preferred_doc="validation")
