...
def train_and_evaluate_for_k(
    ...
    for layer_idx in range(1, num_layers + 1):
        ...
        try:
            ...
            # Step 4: Train final model on ONLY training data (first k pairs) with best params
            print(f"  Training final model on training data only (first {k} pairs)...")
...
def plot_results(
    ...
    output_path: str = "/workspace/results/boolq/boolq_train_size_improve_plot.png",
):
    """
    ...
    plt.title(f"Classifier Performance vs Layer ({aggregation_name}) - BoolQ", fontsize=16, fontweight='bold')
...
if __name__ == "__main__":
    ...
    for k in k_values:
        ...
        # Save to JSON file
        output_dir = "/workspace/results/boolq"
        os.makedirs(output_dir, exist_ok=True)
        output_file = f"{output_dir}/boolq_train_size_k{k}_{aggregation_method.value}_results.json"
