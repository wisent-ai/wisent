...
def plot_individual_aggregation(
    ...
    plt.title(f"Classifier Performance: {aggregation_name} (BoolQ)", fontsize=16, fontweight='bold')
...
def plot_combined_aggregations(
    ...
    num_test: int,
    output_path: str = "/workspace/results_1gpu/boolq/boolq_aggregations_combined_plot.png",
):
    ...
    plt.xlabel("Layer Number", fontsize=14)
    plt.ylabel("Test Accuracy (%)", fontsize=14)
    plt.title("Classifier Performance: All Aggregation Methods (BoolQ)", fontsize=16, fontweight='bold')
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    for agg_method in agg_strategies:
        ...
        # Save to JSON file
        output_dir = "/workspace/results_1gpu/boolq"
        os.makedirs(output_dir, exist_ok=True)
        output_file = f"{output_dir}/boolq_aggregation_{agg_name}_results.json"
        ...
        plot_individual_aggregation(
            ...
            num_test=num_test,
            output_path=f"/workspace/results_1gpu/boolq/boolq_aggregation_{agg_name}_plot.png",
        )
