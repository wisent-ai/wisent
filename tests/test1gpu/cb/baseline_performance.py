...
def load_cb_questions(limit, preferred_doc: str = "validation"):
    ...
    if not docs:
        raise RuntimeError(f"Could not load CB documents from {preferred_doc} source")
...
def evaluate_baseline_performance(
    ...
    questions = load_cb_questions(limit=num_questions, preferred_doc="validation")
