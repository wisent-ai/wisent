...
def plot_individual_aggregation(
    ...
    plt.title(f"Classifier Performance: {aggregation_name} (CB)", fontsize=16, fontweight='bold')
...
def plot_combined_aggregations(
    ...
    num_test: int,
    output_path: str = "/workspace/results_1gpu/cb/cb_aggregations_combined_plot.png",
):
    ...
    plt.xlabel("Layer Number", fontsize=14)
    plt.ylabel("Test Accuracy (%)", fontsize=14)
    plt.title("Classifier Performance: All Aggregation Methods (CB)", fontsize=16, fontweight='bold')
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    num_train = 25  # 400 activations
    num_val = 5    # 100 activations
    num_test = 15   # 112 activations
    n_trials = 10
    ...
    for agg_method in agg_strategies:
        ...
        # Save to JSON file
        output_dir = "/workspace/results_1gpu/cb"
        os.makedirs(output_dir, exist_ok=True)
        output_file = f"{output_dir}/cb_aggregation_{agg_name}_results.json"
        ...
        plot_individual_aggregation(
            ...
            num_test=num_test,
            output_path=f"/workspace/results_1gpu/cb/cb_aggregation_{agg_name}_plot.png",
        )
