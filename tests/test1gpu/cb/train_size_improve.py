"""
Train classifiers on varying numbers of training examples (k = 5, 10, 20, 50, 100, 200)
- Training+Validation from training docs (250 available: k for training + 50 for validation)
- Test from validation docs (56 available)
...
from activation_matrix import load_pairs, create_activations_matrix
...
def train_and_evaluate_for_k(
    ...
    Training docs (250 available): k for training + 50 for validation
    Validation docs (56 available): use for test
...
def plot_results(
    ...
    output_path: str = "/workspace/results/cb/cb_train_size_improve_plot.png",
):
    """
    ...
    plt.title(f"Classifier Performance vs Layer ({aggregation_name}) - CB", fontsize=16, fontweight='bold')
...
if __name__ == "__main__":
    ...
    num_test = 56  # All 56 samples from validation docs
    k_values = [5, 10, 20, 50, 100, 200]  # Max k=200 so k+50=250 fits in training docs
    n_trials = 40
    ...
    for k in k_values:
        ...
        # Save to JSON file
        output_dir = "/workspace/results/cb"
        os.makedirs(output_dir, exist_ok=True)
        output_file = f"{output_dir}/cb_train_size_k{k}_{aggregation_method.value}_results.json"
