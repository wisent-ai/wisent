...
# Load argument_topic task
tasks = get_task_dict(["argument_topic"])
argument_topic_task = tasks["argument_topic"]
...
for split_method in ['test_docs', 'validation_docs', 'train_docs', 'fewshot_docs']:
    if hasattr(argument_topic_task, split_method):
        try:
            docs = list(getattr(argument_topic_task, split_method)())
            if docs:
...
print(f"argument_topic dataset loaded: {len(docs)} samples from {split_used}")
