#!/usr/bin/env python3
"""Generate capability pairs using Qwen3 via Together AI API."""
...
FILENAME = "qwen3_8b_pairs.json"  # Keep consistent naming
API_URL = "https://api.together.xyz/v1/chat/completions"


def call_api(api_key: str, messages: list, max_tokens: int = 500, temperature: float = 0.7) -> str:
...
def generate_opposite_trait(api_key: str, trait: str) -> str:
    ...
    return call_api(api_key, messages, max_tokens=200)
...
def generate_pair(api_key: str, trait: str, opposite_trait: str) -> dict | None:
    ...
    prompt = call_api(api_key, prompt_messages, max_tokens=100, temperature=0.9)
...
def generate_for_capability(api_key: str, capability: str, trait: str, base_dir: str):
    ...
    # Save
    save_data = {
        'model': 'Qwen3-8B',  # Logical model name for consistency
        'actual_model': MODEL_NAME,  # Actual model used
        'trait_description': trait,
...
def main():
    ...
    print(f"Generating pairs for {len(CAPABILITIES)} capabilities using Qwen3 via Together AI")
