...
class PULSERuntimeHooks:
    ...
    PULSE uses a condition vector to determine when to apply steering:
    - Computes cosine similarity between sensor layer hidden state and condition vector
    - If similarity > threshold, applies steering with soft gating
    - Supports per-layer scaling factors
    
    Usage:
        hooks = PULSERuntimeHooks(model, pulse_result)
        hooks.install()
        # ... generate with model ...
    ...
    def _sensor_hook(self, module, input, output):
        ...
        # Get last token hidden state for gating decision
        if hidden_states.dim() == 3:
            sensor_hidden = hidden_states[:, -1, :]  # [batch, hidden]
        else:
            sensor_hidden = hidden_states
        
        self._sensor_activation = sensor_hidden
        
        # Compute gate using condition vector
        if hasattr(self.pulse_result, 'compute_gate'):
            self._current_gate = self.pulse_result.compute_gate(
            ...
            self._current_gate = torch.sigmoid(
                (similarity - self.pulse_result.optimal_threshold) / self.gate_temperature
            )
    ...
    def _steering_hook(self, module, input, output, layer_name):
        ...
        # Get behavior vector for this layer
        ...
        # Apply steering: h' = h + gate * base_strength * layer_scale * behavior_vector
        gate = self._current_gate.to(hidden_states.device)
