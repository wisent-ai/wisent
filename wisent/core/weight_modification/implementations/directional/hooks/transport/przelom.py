"""PRZELOM runtime hooks for attention-transport steering at inference time."""
...
class PrzelomRuntimeHooks:
    """
    Runtime hook system for PRZELOM attention-transport steering.
    ...
    Same pattern as SzlakRuntimeHooks.
    """
        ...
        for layer_idx in self.source_points:
            if layer_idx < len(self._layers):
                hook = self._layers[layer_idx].register_forward_hook(
                    lambda mod, inp, out, li=layer_idx: self._przelom_hook(mod, inp, out, li))
    ...
    def _przelom_hook(self, module, input, output, layer_idx: int):
    ...
    def _apply_displacement(self, hidden_states: torch.Tensor, layer_idx: int) -> torch.Tensor:
        ...
        return h_new.to(original_dtype).reshape(original_shape)
