...
def compute_steering_recommendation(
    ...
    *,
    ...
    Requires either a saved learned config or explicit parameters.
    """
    ...
    if config is not None:
        ...
        if not learned.exists():
            raise ValueError(
                "No learned config found and no config provided. "
                "Pass config=RecommendationConfig(...) explicitly."
            )
        cfg = RecommendationConfig.load(learned)
    return compute_configurable_recommendation(
        metrics, cfg,
...
def compute_per_layer_recommendation(
    ...
    *,
    ...
    for layer, metrics in per_layer_metrics.items():
        rec = compute_steering_recommendation(
            metrics,
            config=config,
            confidence_margin_weight=confidence_margin_weight,
            baseline_confidence=baseline_confidence,
...
def get_method_requirements(method: str) -> Dict[str, Any]:
    ...
    return {
        "min_pairs": defn.optimization_config["min_pairs"],
