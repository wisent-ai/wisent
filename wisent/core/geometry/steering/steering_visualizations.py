...
def create_steering_effect_figure(
    ...
    If classifier is provided, draws the decision boundary showing truthful/untruthful regions.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    # Draw decision boundary if we have space probs (train a 2D classifier for visualization)
    if base_space_probs is not None and steered_space_probs is not None:
        from sklearn.linear_model import LogisticRegression
        # Train a 2D classifier on PCA-projected reference data for visualization
        ...
        # Draw filled contour showing truthful (blue) vs untruthful (red) regions
        cmap_light = ListedColormap(['#FFCCCC', '#CCCCFF'])  # light red, light blue
    ...
    if base_evaluations is not None and steered_evaluations is not None:
        ...
        metrics_text += (
            f"\n\nText Evaluation:\n"
            f"  Base TRUTHFUL: {base_truthful}/{len(base_evaluations)} ({100*base_truthful/len(base_evaluations):.1f}%)\n"
            f"  Steered TRUTHFUL: {steered_truthful}/{len(steered_evaluations)} ({100*steered_truthful/len(steered_evaluations):.1f}%)"
        )

    # Add activation space location stats if available
    if base_space_probs is not None and steered_space_probs is not None:
        base_in_truthful = sum(1 for p in base_space_probs if p >= 0.5)
        steered_in_truthful = sum(1 for p in steered_space_probs if p >= 0.5)
        base_mean_prob = np.mean(base_space_probs)
        steered_mean_prob = np.mean(steered_space_probs)
        metrics_text += (
            f"\n\nActivation Space Location:\n"
            f"  Base in truthful: {base_in_truthful}/{len(base_space_probs)} ({100*base_in_truthful/len(base_space_probs):.1f}%)\n"
            f"  Steered in truthful: {steered_in_truthful}/{len(steered_space_probs)} ({100*steered_in_truthful/len(steered_space_probs):.1f}%)\n"
            f"  Base mean P(truthful): {base_mean_prob:.3f}\n"
            f"  Steered mean P(truthful): {steered_mean_prob:.3f}"
        )

        # Diagnostic: compare activation shift vs text evaluation improvement
        if base_evaluations is not None and steered_evaluations is not None:
            activations_shifted = steered_in_truthful > base_in_truthful
            text_improved = sum(1 for e in steered_evaluations if e == "TRUTHFUL") > sum(1 for e in base_evaluations if e == "TRUTHFUL")
...
def create_per_concept_steering_figure(
    ...
    return create_steering_effect_figure(
        ...
        base_space_probs=base_space_probs,
        steered_space_probs=steered_space_probs,
    )
